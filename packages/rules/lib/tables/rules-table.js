const { evaluate } = require('@ponzujs/eval');
...
class RulesTable {
  ...
  async getValue(rowIndex, columnIndex) {
    ...
    if (value && typeof value === 'string' && value.trim().startsWith('=')) {
      ...
      value = await evaluate(value.trim().substring(1), context);
    }
    return toValue(this.returnParam.type, value);
  }

  async getResult(rowIndex, columnIndex) {
    const value = await this.getValue(rowIndex, columnIndex);
  ...
  async executeContext(paramValues) {
    const rowIndexes =
    ...
    if (this.isMulti) {
      ...
      for (let i = 0; i < rowIndexes.length; i += 1) {
        for (let j = 0; j < columnIndexes.length; j += 1) {
          ...
          results.add(await this.getResult(rowIndexes[i], columnIndexes[j]));
        }
      }
      return [...results];
    }
